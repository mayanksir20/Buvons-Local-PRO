...
const Supplier = () => {
  ...
  const handleEditSupplier = (targetId) => {
    navigate(`/supplier-management/edit-supplier/${targetId}`);
  };

  const handleViewSupplier = (targetId) => {
    navigate(`/supplier-management/view-supplier/${targetId}`);
  };

  const handleAdd = () => {
    navigate("/supplier-management/add-supplier");
  };
  ...
  useEffect(() => {
    ...
    apis
      .get("/suppliersList", config)
      .then((res) => {
        if (res.data.success === true) {
          setSupplierList(res.data.data);
          setSupplierTableData(res.data.data);
          setHideFilter("");
        } else {
    ...
    apis
      ...
      .then((res) => {
        if (res.data.success) {
          ...
          setStatus(value ? "1" : "0");
          const filterData = supplierTableData.map((el) => {
                          ...
                          <table className="table table-striped m-0">
                            <thead>
                              <tr>
                                ...
                                <th className="tableNameBox">
                                  {t(
                                    "admin.supplier_management.list.table_col4"
                                  )}
                                </th>
